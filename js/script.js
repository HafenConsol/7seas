...
async function fetchMarketData() {
  try {
    ...
    const response = await fetch(
      "https://economia.awesomeapi.com.br/last/USD-BRL,EUR-BRL,BTC-USD",
    );
    ...
    // Bitcoin (BTC)
    const btc = parseFloat(data.BTCUSD.bid).toLocaleString("en-US", {
      style: "currency",
      currency: "USD",
    });
...
function simulateTracking() {
  const input = document
    ...
    .value.toUpperCase()
    .trim(); // Converte pra maiúscula e tira espaços
...
// 2. Calculadora de Frete (Peso Cubado - Aéreo)
function calculateFreight() {
  // Pega os valores dos inputs de Dimensão
  ...
  const btn = document.querySelector("#calc .terminal-action-btn");

  // Validação de Segurança: Verifica se o usuário preencheu tudo certo
  if (isNaN(l) || isNaN(w) || isNaN(h) || l <= 0 || w <= 0 || h <= 0) {
    alert(
      "SYSTEM ERROR: Please enter valid dimensions (L x W x H) greater than zero.",
    );
    return;
  }

  // Efeito Visual de Processamento (Terminal Vibe)
  const originalText = btn.innerHTML;
  btn.innerHTML = '<i class="fas fa-cog fa-spin"></i> CALCULATING...';
  btn.style.opacity = "0.7";
  resultBox.style.display = "none"; // Esconde resultado antigo

  // Simula o tempo de processamento do terminal (1.5 segundos)
  setTimeout(() => {
    btn.innerHTML = originalText;
    btn.style.opacity = "1";

    // A MÁGICA: Fórmula IATA de Peso Cubado
    const chargeableWeight = (l * w * h) / 6000;

    // Injeta o resultado no HTML com 2 casas decimais
    weightValue.innerText = chargeableWeight.toFixed(2);

    // Exibe o resultado com uma transição suave
    resultBox.style.display = "block";
    resultBox.style.animation = "fadeIn 0.5s";
  }, 1500);
}
...
// O "Ouvinte" de cliques
hamburgerBtn.addEventListener("click", () => {
  // O toggle funciona como um interruptor: se não tem a classe 'active', ele coloca. Se já tem, ele tira.
